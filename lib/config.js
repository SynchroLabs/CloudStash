...
var logger = require('bunyan').createLogger({name: "Config"});
...
exports.getConfig = function(configFile, overrides)
{
    ...
    // nconf.env();
    //
    // Parse env vars that start with "MANTABOX__", with segments separated by "__", remove the prefix, put the values
    ...
    {
        // We're only going to take env variables that start with "MANTABOX__"...
        //
        return key.indexOf("MANTABOX__") == 0;
    }).forEach(function (key) 
    {
        ...
        // the "MANTABOX" part, and then create a normal colon-separated path from the remaining parts...
        //
    ...
    else if (process.env['MANTABOX_CONFIG'])
    {
        conf.configDetails = "Using configuration file specified in MANTABOX_CONFIG: " + process.env['MANTABOX_CONFIG'];
        conf.file('config.json', process.env['MANTABOX_CONFIG']);
    }
    else
    {
