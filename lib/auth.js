...
exports.addAuthMiddleware = function(server, tokenManager)
{
    ...
    server.post('/login', restify.queryParser(), restify.bodyParser(), restify.authorizationParser(), function(req, res, next)
    {
        ...
        if (userAccount)
        {
            var token = tokenManager.createAuthToken(req.params.client_id, userAccount.account_id);
            res.send(token);
        }
        else
        {
    ...
    server.post('/1/connect', restify.bodyParser(), function(req, res, next)
    {
        ...
        if (userAccount)
        {
            ...
            //
            var token = tokenManager.createAuthToken(userAccount.app_id, userAccount.account_id);
    ...
    server.post('/oauth2/authorize', restify.bodyParser(), function(req, res, next)
    {
        ...
        if (userAccount)
        {
            ...
            if (req.params.response_type === 'code')
            {
                ...
                //
                var code = tokenManager.createAuthCode(userAccount.app_id, userAccount.account_id);
    ...
    return ['/login', '/oauth2/authorize', '/oauth2/token', '/1/connect'];
}
