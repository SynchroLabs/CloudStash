    ...
    var bridge =
    {
        ...
        isAnyCursorItemNewer: function(user, path, recursive, cursorItem, callback)
        {
            ...
            else if (driver.getLatestCursorItem)
            {
                ...
                {
                    callback(err, bridge.isCursorItemNewer(latestCursorItem, cursorItem));
                });
            }
            else // Fall back to traverse (brute force)
            {
                function onEntry(entry)
                {
                    return bridge.isCursorItemNewer(getCursorItem(entry), cursorItem);
                }
        ...
        getObjectMetaDataWithRetry: function(user, filename, callback)
        {
                        ...
                        driver.putObject(user, filename, function(err, writeStream)
                        {
                            ...
                            function(err)
                            {
                                ...
                                root.deleteDirectory(user, uploadDirPath, function(err)
                                {
                                    ...
                                    bridge.getObjectMetaDataWithRetry(user, filename, function(err, srcEntry)
                                    {
