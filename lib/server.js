...
module.exports = function(_jwtSecret, config)
{
    ...
    var dropBoxEntriesLimit = 4; // !!! real number is: 725
    ...
    */
    ...
    function filesListFolder(req, res, next)
    {
        ...
        driver.listDirectory(req.user, apiArgs.path, apiArgs.recursive, dropBoxEntriesLimit, null, function(err, items, hasMore)
        {
            if (err)
            {
                // !!!
            ...
            { 
                cursor.lastItem = 
                {
                    "server_modified": items[items.length-1]["server_modified"],
                    "path_display": items[items.length-1]["path_display"]
                }
            } 

            var cursorString = new Buffer(JSON.stringify(cursor)).toString("base64");
    ...
    function filesListFolderContinue(req, res, next)
    {
        ...
        var cursor = JSON.parse(Buffer(apiArgs.cursor, 'base64').toString());

        driver.listDirectory(req.user, cursor.path, cursor.recursive, dropBoxEntriesLimit, cursor.lastItem, function(err, items, hasMore)
        {
            if (err)
            {
                // !!!
            ...
            var newCursorString = new Buffer(JSON.stringify(cursor)).toString("base64");
