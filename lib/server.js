...
module.exports = function(_jwtSecret, config)
{
    ...
    try {
        throw new dbErr.DropboxError("other", null, "Dropbox sucks");
    }
    catch (err) {
        log.error(err);
        log.info("Dropbox Error", err.dropboxError);
    }
    ...
    */
    ...
    {
        // !!! assert operation = move | copy | delete
        //
                ...
                else
                {
                    ...
                    driver.listDirectory(req.user, workItem.srcEntry.path_display, false, null, null, function(err, entries)
                    {
                        ...
                        else // Empty folder
                        {
                            ...
                            if ((operation === "move") || (operation === "delete"))
                            {
                                // Schedule the folder for deletion now (it was empty)
                                //
    ...
    var maxInteractive = 10000; // More than this many operations in a non-batch call will result in too_many_files
