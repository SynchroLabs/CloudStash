...
module.exports = function(_jwtSecret, config)
{
    ...
    {
        var apiArgs = getApiArgs(req);
        req.log.info("List folders for account_id: %d at path: %s", req.user.account_id, apiArgs.path);

        driver.listDirectory(apiArgs.path, function(err, items)
        {
    ...
    {
        var apiArgs = getApiArgs(req);
        req.log.info("Delete file for account_id: %d at path: %s", req.user.account_id, apiArgs.path);

        driver.deleteObject(apiArgs.path, function(err, item)
        {
            res.send(item);
            next();
        });
    ...
    {
        var apiArgs = getApiArgs(req);
        req.log.info("Create folder for account_id: %d at path: %s", req.user.account_id, apiArgs.path);

        driver.createDirectory(apiArgs.path, function(err, item)
        {
            res.send(item);
            next();
        });
    ...
    {
        var apiArgs = getApiArgs(req);
        req.log.info("Copy for account_id: %d from path: %s to path: %s", req.user.account_id, apiArgs.from_path, apiArgs.to_path);

        driver.copyObject(apiArgs.from_path, apiArgs.to_path, function(err, item)
        {
    ...
    {
        var apiArgs = getApiArgs(req);
        req.log.info("Move for account_id: %d from path: %s to path: %s", req.user.account_id, apiArgs.from_path, apiArgs.to_path);

        driver.moveObject(apiArgs.from_path, apiArgs.to_path, function(err, item)
        {
            res.send(item); // This appears to be the source item
            next();
        });
