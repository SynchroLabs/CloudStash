...
module.exports = function(_jwtSecret, config)
{
    ...
    // Set up auth strategy from config
    //
    var strategy = config.get('AUTH_STRATEGY');
    var authStrategy;
    if (strategy === 'local')
    {
        authStrategy = require('./auth-local');
        authStrategy.addStrategyMiddleware();
    }
    else if (strategy === 'saml')
    {
        authStrategy = require('./auth-saml');
        authStrategy.addStrategyMiddleware(config.get('SAML_CALLBACK'), config.get('SAML_ENTRYPOINT'), config.get('SAML_ISSUER'));
    }

    log.info("Adding auth middleware for strategy:", strategy);
    var exemptions = auth.addAuthMiddleware(server, strategy, tokenMgr);
    ...
    {
        driverModule = require('../drivers/file-driver');
    }
    else if (driverConfig.provider === "aws")
    {
        driverModule = require('../drivers/aws-driver');
    }
    else if (driverConfig.provider === "azure")
    {
        driverModule = require('../drivers/azure-driver');
    }
    else if (driverConfig.provider === "manta")
    {
        driverModule = require('../drivers/manta-driver');
    }
    else
    {
    ...
    var driver = new driverModule(driverConfig, config);
    var driverBridge = require('../drivers/bridge')(config, driver);
    var bridge = require('./user-bridge')(config, driverBridge);
    var root = require('./root')(config, bridge);
    var bulk = require('./bulk')(config, bridge);
    ...
    function passportUserProfileToDropbox (profile)
    {
        // http://passportjs.org/docs/profile

        var dropboxUser = 
        {
            // We should probably combine the provider with the ID at some point, but for now that's gonna mess with our
            // namespace on the file systems, so we'll punt.
            //
            // account_id: lodash.padEnd(profile.provider + ':' + profile.id, 40),
            //
            account_id: lodash.padEnd(profile.id, 40), // Dropbox really wants exactly 40 chars
            name: {
                display_name: profile.displayName,
                given_name: profile.name.givenName,
                surname: profile.name.familyName,
                familiar_name: profile.name.givenName,
                abbreviated_name: "TU"
            },
        ...
        log.info("Returning Dropbox user:", dropboxUser);
    ...
    server.post('/2/users/get_account', restify.bodyParser(), function(req, res, next)
    {
        ...
        authLocal.getAccount(apiArgs.account_id, function(err, profile)
        {
            log.info("get account", err, profile);
            if (err)
            {
            ...
            {
                next(dbErr.returnDropboxErrorNew(res, "no_account", null, "Account not found"));
            }
            else
            {
                res.send(passportUserProfileToDropbox(profile));
            }
        }); 
        next();
    });
    ...
    server.post('/2/users/get_current_account', function(req, res, next)
    {
        ...
        authStrategy.getAccount(req.user.account_id, function(err, profile)
        {
            log.info("get current account", err, profile);
            if (err)
            {
                next(dbErr.returnDropboxError(res, err));
            }
    ...
    function filesDownload(req, res, next)
    {
        ...
        bridge.getObjectMetaData(req.user, apiArgs.path, function(err, entry)
        {
            if (err)
            {
            ...
            {
                bridge.getObject(req.user, apiArgs.path, req.headers, function(err, stream, statusCode, statusMessage, headers)
                {
                    if (err)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            req.log.info("Setting content length to entry size:", entry.size);
                            respHeaders['Content-Length'] = entry.size;
                        }

                        req.log.info("Writing response with code:", respCode);
                        ...
                        if (stream)
                        {
                            ...
                            req.log.info("Piping stream");
                            stream.pipe(res); // Pipe store object contents to output
                        }
                        else
                        {
                            req.log.info("No stream, ending");
                            res.end();
                        }
    ...
    function filesUpload(req, res, next)
    {
        ...
        bulk.validateOperation(req.user, null, apiArgs.path, "upload", mode, null, function(err, itemCount, srcEntry, dstPath)
        {
            ...
            else
            {
                ...
                bridge.putObject(req.user, dstPath, req, function(err, details)
                {
                    if (err)
                    {
                        req.log.error(err, "Error on upload");
                        return next(dbErr.returnDropboxError(res, err));
                    }
                    ...
                    bridge.getObjectMetaDataWithRetry(req.user, dstPath, false, function(err, entry)
                    {
                        if (err)
                        {
    ...
    function uploadSessionStart(req, res, next)
    {
        ...
        bridge.startMultipartUpload(req.user, req, function(err, sessionId)
        {
            if (err)
            {
            ...
            log.info("Upload start succeeded");
    ...
    function uploadSessionAppend(apiArgs, req, res, next)
    {
        ...
        bridge.multipartUpload(req.user, req, apiArgs.cursor.session_id, apiArgs.cursor.offset, function(err, details)
        {
            if (err)
            {
            ...
            log.info("Upload session append succeeded");
            res.send();
            next();
        });
    }
    ...
    function uploadSessionFinish(req, res, next)
    {
        ...
        bulk.validateOperation(req.user, null, apiArgs.commit.path, "upload", mode, null, function(err, itemCount, srcEntry, dstPath)
        {
            ...
            else
            {
                ...
                bridge.multipartUpload(req.user, req, apiArgs.cursor.session_id, apiArgs.cursor.offset, function(err, stream)
                {
                    if (err)
                    {
                    ...
                    log.info("Upload session append of final part succeeded");
    ...
    */
    ...
    // NOTE: This API endpoint (/2/files/list_folder/longpoll) does not use Authorization, it gets the user
    //       information (account_id and app_id) from the cursor.  This is per the Dropbox spec.
    //
    function filesListFolderLongPoll(req, res, next)
    {
        ...
        req.log.info("List folder long poll for account_id: %s", cursor.user.account_id);
        ...
        async.whilst(
            ...
            function(callback) 
            {
                bridge.isAnyCursorItemNewer(cursor.user, cursor.path, cursor.recursive, cursor.lastItem, function(err, isNewer)
                {
                    if (err)
                    {
                        callback(err);
    ...
    function filesCreateFolder(req, res, next)
    {
        ...
        bridge.createDirectory(req.user, apiArgs.path, function(err)
        {
            if (err)
            {
            ...
            {
                bridge.getObjectMetaDataWithRetry(req.user, apiArgs.path, true, function(err, entry)
                {
                    if (err)
                    {
        ...
        bulk.validateOperation(req.user, apiArgs.from_path, apiArgs.to_path, operation, mode, _maxInteractive, function(err, itemCount, srcEntry, dstPath)
        {
            ...
            else
            {
                ...
                bulk.doOperation(req, operation, [{ srcEntry: srcEntry, dstPath: dstPath }], function(err)
                {
                    ...
                    // srcEntry[".tag"] will indicate file or folder for metadata call
                    bridge.getObjectMetaDataWithRetry(req.user, dstPath, srcEntry[".tag"] === "folder", function(err, entry)
                    {
                        if (err)
                        {
    ...
    function filesMoveCopyBatch(req, res, next)
    {
        ...
        [
            ...
            function(callback) // Get entry info for dstPath items
            {
                ...
                {
                    // workItem.srcEntry[".tag"] indicates file or folder of src, and dst should be the same...
                    bridge.getObjectMetaDataWithRetry(req.user, workItem.dstPath, workItem.srcEntry[".tag"] === "folder", function(err, entry)
                    {
                        if (err)
                        {
                            req.log.error("Error getting metadata on to_path after %s", operation, err);
    ...
    function filesUploadSessionFinishBatch(req, res, next)
    {
        ...
        [
            ...
            function(callback) // Write final status (results) to job file...
            {
                ...
                req.log.info("Writing job status complete for job:", jobId);
                ...
                root.putObject(req.user, "/jobs/" + jobId, status, function(err)
                {
                    ...
                    req.log.info("Wrote job status complete for job:", jobId);
                    callback(err);
                });
            }
        ],
    ...
    function filesJobBatchCheck(req, res, next)
    {
        ...
        req.log.info("Getting job status for jobId:", apiArgs.async_job_id);
    ...
    function filesGetMetaData(req, res, next)
    {
        ...
        bridge.getMetaData(req.user, apiArgs.path, null, function(err, item)
        {
            if (err)
            {
