...
module.exports = function(_jwtSecret, config)
{
    ...
    // server.post('/files/upload_session/append', notImplemented);
    server.post('/files/upload_session/append_v2', notImplemented);
    // server.post('/files/upload_session/finish', notImplemented);
    ...
    // server.post('/files/upload_session/start', notImplemented);

    server.post('/files/upload_session/start', function (req, res, next)
    {
        var apiArgs = getApiArgs(req);
    ...
    {
        var apiArgs = getApiArgs(req);
        req.log.info("Append upload session %s, offset: %n", apiArgs.session_id, apiArgs.offset);

        driver.multipartUpload(req.user, apiArgs.session_id, apiArgs.offset, function(err, stream)
        {
            if (err)
            {
    ...
    server.post('/files/upload_session/finish', function (req, res, next)
    {
        var apiArgs = getApiArgs(req);
        ...
        driver.multipartUpload(req.user, apiArgs.cursor.session_id, apiArgs.cursor.offset, function(err, stream)
        {
            ...
            pipeRequest(req, stream, function(err, details)
            {
                ...
                else
                {
                    ...
                    driver.finishMultipartUpload(req.user, apiArgs.cursor.session_id, apiArgs.commit.path, function(err)
                    {
                        if (err)
                        {
