...
module.exports = function(_jwtSecret, config)
{
    ...
    //
    function getObjectMetaDataWithRetry(user, filename, callback)
    {
        ...
        function(callback)
        {
            driver.getObjectMetaData(user, filename, function(err, entry)
            {
                if (!err && !entry)
                {
        ...
        driver.putObject(req.user, apiArgs.path, function(err, stream)
        {
            ...
            pipeRequest(req, stream, function(err, details)
            {
                ...
                else
                {
                    ...
                    //
                    log.debug("Put response:", details);
                    ...
                    */
                    ...
                    getObjectMetaDataWithRetry(req.user, apiArgs.path, function(err, entry)
                    {
                        if (err)
                        {
                ...
                else
                {
                    ...
                    driver.putObject(user, filename, function(err, writeStream)
                    {
                        async.eachSeries(entries, function (entry, callback) 
                        {
                            ...
                            driver.getObject(stripAppId(user), currentFile, function(err, readStream)
                            {
                        ...
                        function(err)
                        {
                            ...
                            deleteAccountDirectory(user, uploadDirPath, function(err)
                            {
                                ...
                                getObjectMetaDataWithRetry(user, filename, function(err, srcEntry)
                                {
    ...
    */
                ...
                try 
                {
                    ...
                    else // move/copy
                    {
                        ...
                        else // dest entry doesn't exist
                        {
                            ...
                            driver.createDirectory(user, createDstPath, function(err)
                            {
        ...
        var q = async.queue(function(workItem, done) 
        {
            ...
            if (workItem.srcEntry[".tag"] === "file")
            {
                ...
                if (operation === "copy")
                {
                    ...
                    driver.copyObject(req.user, workItem.srcEntry.path_display, workItem.dstPath, function(err)
                    {
                        if (err)
                        {
                ...
                else if (operation === "move")
                {
                    ...
                    driver.moveObject(req.user, workItem.srcEntry.path_display, workItem.dstPath, function(err)
                    {
                        if (err)
                        {
                ...
                else if (operation === "delete")
                {
                    ...
                    driver.deleteObject(req.user, workItem.srcEntry.path_display, function(err)
                    {
                        if (err)
                        {
                    ...
                    listFolderUsingCursor(req.user, workItem.srcEntry.path_display, false, null, null, function(err, entries)
                    {
                        ...
                        else // Empty folder
                        {
                            ...
                            if ((operation === "move") || (operation === "copy"))
                            {
                                ...
                                //
                                driver.createDirectory(req.user, workItem.dstPath, function(err)
                                {
                                    if (err)
                                    {
        ...
        validateBulkOperation(req.user, apiArgs.from_path, apiArgs.to_path, operation, mode, maxInteractive, function(err, itemCount, srcEntry, dstPath)
        {
            ...
            else
            {
                ...
                doBulkOperation(req, operation, [{ srcEntry: srcEntry, dstPath: dstPath }], function(err)
                {
                    ...
                    getObjectMetaDataWithRetry(req.user, dstPath, function(err, entry)
                    {
                        if (err)
                        {
    ...
    {
        driver.getObject(stripAppId(user), path, function(err, stream)
        {
            if (err)
            {
                callback(err);
    ...
    function filesMoveCopyBatch(req, res, next)
    {
        ...
        [
            ...
            function(callback) // Get entry info for dstPath items
            {
                ...
                {
                    getObjectMetaDataWithRetry(req.user, workItem.dstPath, function(err, entry)
                    {
                        if (err)
                        {
                            req.log.error("Error getting metadata on to_path after %s", operation, err);
