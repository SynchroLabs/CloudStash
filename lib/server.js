...
module.exports = function(_jwtSecret, config)
{
    ...
    */
    ...
    function filesCopyMove(req, res, next)
    {
        ...
        req.log.info("%s (%s) for account_id: %s from path: %s to path: %s", req.route.path, operation, req.user.account_id, apiArgs.from_path, apiArgs.to_path);
    ...
    function filesMoveCopyBatch(req, res, next)
    {
        ...
        req.log.info("%s (%s) for account_id", req.route.path, operation, req.user.account_id);
        ...
        [
            ...
            function(callback) // Validate bulk operation
            {
                async.eachLimit(apiArgs.entries, maxConcurrency, function(entry, callback)
                {
                    // Validate entry.path
                    //
                    validateBulkOperation(req.user, entry.from_path, entry.to_path, operation, null, null, function(err, itemCount, srcEntry, dstPath)
                    {
                        if (!err)
                        {
            ...
            function(callback) // Do bulk operation
            {
                ...
                doBulkOperation(req, operation, workItems, function(err)
                {
                    if (err)
                    {
                        req.log.error("Error on batch %s:", operation, err);
                        // !!! Write err to job file
                        // return next(dbErr.returnDropboxError(res, err));
                    }
    ...
    server.post('/files/copy', restify.bodyParser(), filesCopyMove);
    server.post('/files/move', restify.bodyParser(), filesCopyMove);
