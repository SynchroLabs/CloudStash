...
module.exports = function(_jwtSecret, config)
{
    ...
    */
    ...
    function isItemNewerThan(item1, item2)
    {
        return (!item1 || (driver.getEntrySortKey(item1) < driver.getEntrySortKey(item2)));
    }
    ...
    function filesListFolderLongPoll(req, res, next)
    {
        ...
        async.whilst(
            ...
            function(callback) 
            {
                driver.getLatestObject(req.user, cursor.path, cursor.recursive, function(err, entry)
                {
                    if (err)
                    {
                        callback(err);
                    }
                    else if (entry && isItemNewerThan(cursor.lastItem, entry))
                    {
                        req.log.info("Found item later than cursor item:", entry);
                        newItemFound = true;
                        callback();
                    }
                    else
