    ...
    */
    ...
    function encodeCursor(path, recursive, limit, lastItem)
    {
        ...
        { 
            cursor.lastItem = 
            {
                "server_modified": lastItem["server_modified"],
                "path_display": lastItem["path_display"]
            }
        } 
    ...
    {
        return JSON.parse(Buffer(apiArgs.cursor, 'base64').toString());
    }
    ...
    function filesListFolder(req, res, next)
    {
        ...
        driver.listDirectory(req.user, apiArgs.path, apiArgs.recursive, limit, null, function(err, items, hasMore)
        {
            if (err)
            {
                // !!!
            ...
            var lastItem = (items && items.length) ? items[items.length-1] : null;
            var cursorString = encodeCursor(apiArgs.path, !!apiArgs.recursive, limit, lastItem)
    ...
    function filesListFolderContinue(req, res, next)
    {
        ...
        driver.listDirectory(req.user, cursor.path, cursor.recursive, cursor.limit, cursor.lastItem, function(err, items, hasMore)
        {
            if (err)
            {
                // !!!
            ...
            var lastItem = (items && items.length) ? items[items.length-1] : null;
            var newCursorString = encodeCursor(cursor.path, cursor.recursive, cursor.limit, lastItem)
    ...
    function filesListFolderGetLatestCursor(req, res, next)
    {
        ...
        driver.getLatestObject(req.user, apiArgs.path, apiArgs.recursive, function(err, entry)
        {
            var cursorString = encodeCursor(apiArgs.path, apiArgs.recursive, limit, entry);
    ...
    function filesListFolderLongPoll(req, res, next)
    {
        ...
        var timeout = apiArgs.timeout || 30;

        // !!! We can use driver.getLatestObject to see if an object that meets the cursor criteria has shown
        //     up, and if so, we can then call driver.listDirectory (exactly as with list_folder/continue),
        //     otherwise we pause for a small interval and try again, until we hit the timeout.
        //
