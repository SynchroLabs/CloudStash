...
module.exports = function(_jwtSecret, config)
{
    ...
    // via req.user.  
    //
    server.use(restifyJwt({ secret: _jwtSecret}).unless({path: [/\/public\/?.*/, '/login', '/oauth2/authorize', '/oauth2/token']}));
    ...
    server.post('/login', restify.queryParser(), restify.bodyParser(), restify.authorizationParser(), function(req, res, next)
    {
        ...
        //
        // !!! No way to get client id with basic auth, so maybe we don't need/want this?
        //
        ...
        if (userAccount)
        {
            var token = jwt.sign(userAccount, _jwtSecret);
            res.send(token);
        }
        else
        {
    ...
    server.get('/oauth2/authorize', restify.queryParser(), function(req, res, next)
    {
        ...
        //
        req.log.info("Authorize (get) params:", req.params);
    ...
    server.post('/oauth2/authorize', restify.bodyParser(), function(req, res, next)
    {
        ...
        //
        req.log.info("Form posted for email: %s, state: %s", req.params.email, req.params.state);
        ...
        if (userAccount)
        {
            ...
            if (req.params.response_type === 'code')
            {
                ...
                //
                var code = jwt.sign({ type: "authenticationCode", app_id: userAccount.appId, account_id: userAccount.account_id }, _jwtSecret);
    ...
    server.post('/oauth2/token', restify.queryParser(), restify.bodyParser(), restify.authorizationParser(), function(req, res, next)
    {
        ...
        //
        var accessCode = jwt.verify(req.params.code, _jwtSecret);
        if (accessCode.type === 'authenticationCode')
        {
            if (accessCode.app_id === req.params.client_id)
            {
                if (account.validateApp(req.params.client_id, req.params.client_secret))
                {
                    // Winner!  Return bearer token.
                }
                else
                {
            ...
            {
                // !!! client_id specified did not match app_id in auth code
            }
        }
        else
        {
    ...
    */
    ...
    //
    // !!! Should use new driver traverse instead (move findItems logic to here)
    //
