...
module.exports = function(_jwtSecret, config)
{
    ...
    // via req.user.  
    //
    server.use(restifyJwt({ secret: _jwtAuthTokenSecret}).unless({path: [/\/public\/?.*/, '/login', '/oauth2/authorize', '/oauth2/token']}));
    ...
    server.post('/oauth2/authorize', restify.bodyParser(), function(req, res, next)
    {
        ...
        if (userAccount)
        {
            ...
            if (req.query.state)
            {
                redirect += "&state" + req.query.state;
            }
    ...
    */
    ...
    function filesListFolderGetLatestCursor(req, res, next)
    {
        ...
        // !!! Could use generic driver traverse function and brute force this *unless* driver implemented a 
        //     getLatestCursorItem (which it would only do if it could do so more efficiently than brute force).
        //
    ...
    function filesListFolderLongPoll(req, res, next)
    {
        ...
        async.whilst(
            ...
            {
                // !!! Could use generic driver traverse function and brute force this *unless* driver implemented a 
                //     getLatestCursorItem (and it would only do so if it could implement it more efficiently than brute force).
                //
                // !!! A more specific "isAnyItemNewer" might be able to be implemented more efficiently by the driver
                //     (we don't care about the latest item, just that any item is newer than our cursor).  So we could try
                //     isAnyItemNewer, fall back to getLatestCursorItem, and fall back to traverse with brute force.
                //
            ...
            function(srcEntry, callback) // Ensure entry count is less than maxItems if maxItems specified
            {
                ...
                if (maxItems)
                {
                    ...
                    else
                    {
                        ...
                        driver.traverseDirectory(user, srcpath, onEntry, function(err, stopped)
                        {
                            if (stopped)
                            {
        ...
        var q = async.queue(function(workItem, done) 
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    driver.listDirectory(req.user, workItem.srcEntry.path_display, false, null, null, function(err, entries)
                    {
                        if (err)
                        {
                            req.log.error("Error on list folder:", err);
    ...
    function filesCopy(req, res, next)
    {
        ...
        validateBulkOperation(req.user, apiArgs.from_path, apiArgs.to_path, "copy", mode, maxInteractive, function(err, itemCount, srcEntry, dstPath)
        {
            if (err)
            {
                req.log.error("Error validating copy:", err);
                return next(dbErr.returnDropboxError(res, err));
            }
            else
            {
                log.info("Copy bulk validation:", itemCount, srcEntry, dstPath);

                doBulkOperation(req, "copy", [{ srcEntry: srcEntry, dstPath: dstPath }], function(err)
                {
                    if (err)
                    {
                        req.log.error("Error on copy:", err);
                    ...
                    driver.getObjectMetaData(req.user, dstPath, function(err, entry)
                    {
                        if (err)
                        {
                            req.log.error("Error getting metadata on to_path after copy", err);
                            return next(dbErr.returnDropboxError(res, err));
                        }
                        else
                        {
    ...
    function filesMove(req, res, next)
    {
        ...
        validateBulkOperation(req.user, apiArgs.from_path, apiArgs.to_path, "move", mode, maxInteractive, function(err, itemCount, srcEntry, dstPath)
        {
            if (err)
            {
                req.log.error("Error validating move:", err);
                return next(dbErr.returnDropboxError(res, err));
            }
            else
            {
                log.info("Move bulk validation:", itemCount, srcEntry, dstPath);

                doBulkOperation(req, "move", [{ srcEntry: srcEntry, dstPath: dstPath }], function(err)
                {
                    if (err)
                    {
                        req.log.error("Error on move:", err);
    ...
    function putObject(user, path, content, callback)
    {
        ...
        driver.putObject({account_id: user.account_id}, path, function(err, writeStream)
        {
            if (err)
            {
                callback(err);
    ...
    function getObject(user, path, callback)
    {
        driver.getObject({account_id: user.account_id}, path, function(err, stream)
        {
            if (err)
            {
                callback(err);
    ...
    function deleteObject(user, path, callback)
    {
        driver.deleteObject({account_id: user.account_id}, path, function(err)
        {
            callback(err);
        });
    }
    ...
    //
    // !!! filesCopy/filesMove above can probably be combined also.
    //
    ...
    function filesMoveCopyBatch(req, res, next)
    {
        ...
        req.log.info("Operation:", operation);

        // !!! Implement move/copy batch
        //
    }
    ...
    function filesDeleteBatch(req, res, next)
    {
        ...
        [
            function(callback) // Write in_progress status to job file
            {
                ...
                putObject(req.user, "/jobs/" + jobId, response, function(err)
                {
                    if (err)
                    {
            ...
            function(callback) // Validate bulk operation
            {
                async.eachLimit(apiArgs.entries, maxConcurrency, function(entry, callback)
                {
                    // Validate entry.path
                    //
                    validateBulkOperation(req.user, entry.path, null, "delete", null, null, function(err, itemCount, srcEntry)
                    {
                        if (!err)
                        {
                ...
                function (err) 
                {
                    if (err)
                    {
                        req.log.error("Error validating batch delete:", err);
            ...
            function(callback) // Do bulk operation
            {
                ...
                doBulkOperation(req, "delete", workItems, function(err)
                {
                    if (err)
                    {
                        req.log.error("Error on batch delete:", err);
                        // !!! Write err to job file
                        // return next(dbErr.returnDropboxError(res, err));
                    }
            ...
            function(callback) // Write final status (results) to job file...
            {
                ...
                putObject(req.user, "/jobs/" + jobId, status, function(err)
                {
                    if (err)
                    {
    ...
    function filesJobBatchCheck(req, res, next)
    {
        ...
        getObject(req.user, "/jobs/" + apiArgs.async_job_id, function(err, object)
        {
            if (err)
            {
            ...
            {
                if (object[".tag"] === "complete")
                {
                    // If "complete", delete job file
                    req.log.info("%s - job is complete, delete job", req.route.path);
                    deleteObject(req.user, "/jobs/" + apiArgs.async_job_id, function (err)
                    {
    ...
    //
    // !!! Should we just use traverse here and do the result accumulation/limiting here?  Is there any
    //     chance a driver is really going to do anything better than that?
    //
    function filesSearch(req, res, next)
    {
        ...
        driver.findItems(req.user, apiArgs.path, isMatch, start, limit, function(err, items, hasMore, nextStart)
        {
            if (err)
            {
                req.log.error("Error on get metadata:", err);
            ...
            {
                res.send({ matches: items, more: hasMore, start: nextStart });
            }
            next();
        });
    }

    server.post('/files/list_folder', restify.bodyParser(), filesListFolder);
    server.post('/files/list_folder/continue', restify.bodyParser(), filesListFolderContinue);
    server.post('/files/list_folder/get_latest_cursor', restify.bodyParser(), filesListFolderGetLatestCursor);
    server.post('/files/list_folder/longpoll', restify.bodyParser(), filesListFolderLongPoll);
    server.post('/files/delete', restify.bodyParser(), filesDelete);
    server.post('/files/create_folder', restify.bodyParser(), filesCreateFolder);
    server.post('/files/copy', restify.bodyParser(), filesCopy);
    server.post('/files/move', restify.bodyParser(), filesMove);
    server.post('/files/get_metadata', restify.bodyParser(), filesGetMetaData);
    server.post('/files/search', restify.bodyParser(), filesSearch);

    server.post('/files/copy_batch', restify.bodyParser(), filesMoveCopyBatch);
    server.post('/files/move_batch', restify.bodyParser(), filesMoveCopyBatch);
    server.post('/files/delete_batch', restify.bodyParser(), filesDeleteBatch);

    // The batch/check functions are actually all the same underlying implementation
    //
    server.post('/files/copy_batch/check', restify.bodyParser(), filesJobBatchCheck);
    server.post('/files/delete_batch/check', restify.bodyParser(), filesJobBatchCheck);
    server.post('/files/move_batch/check', restify.bodyParser(), filesJobBatchCheck);
    server.post('/files/upload_session/finish_batch/check', restify.bodyParser(), filesJobBatchCheck);
