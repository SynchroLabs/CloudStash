...
module.exports = function(_jwtSecret, config)
{
    ...
    function returnParameterValidationError(req, err)
    {
    ...
    function filesDownload(req, res, next)
    {
        ...
        {
            return next(returnParameterValidationError(req, err));
        }
        ...
        driver.getObjectMetaData(req.user, apiArgs.path, function(err, entry)
        {
            ...
            {
                // !!! Pass cache control headers (If-Modified-Since, etc), propagate return code for cache hits (and test).
                //
                // !!! If Range request, let driver know (pass header?, more explicit?)
                //
                driver.getObject(req.user, apiArgs.path, function(err, stream, headers)
                {
                    if (err)
                    {
                    ...
                    else if (stream)
                    {
                        ...
                        // !!! For file driver, we could compose some of these headers.  For Content-Type, we could get it from the file name
                        //     using: https://www.npmjs.com/package/mime-types
                        ...
                        req.log.info("Headers:", respHeaders);
        ...
        bridge.multipartUpload(req.user, apiArgs.cursor.session_id, apiArgs.cursor.offset, function(err, stream)
        {
            ...
            csUtil.pipeRequest(req, stream, function(err, details)
            {
                ...
                else
                {
                    ...
                    bridge.finishMultipartUpload(req.user, apiArgs.cursor.session_id, apiArgs.commit.path, function(err, item)
                    {
                        if (err)
                        {
    ...
    */
    ...
    function filesCopyMove(req, res, next)
    {
        ...
        bulk.validateOperation(req.user, apiArgs.from_path, apiArgs.to_path, operation, mode, maxInteractive, function(err, itemCount, srcEntry, dstPath)
        {
            if (err)
            {
    ...
    function filesDelete(req, res, next)
    {
        ...
        bulk.validateOperation(req.user, apiArgs.path, null, "delete", null, maxInteractive, function(err, itemCount, srcEntry)
        {
            if (err)
            {
